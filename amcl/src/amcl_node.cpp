        ...
        first_reconfigure_call_(true)
{
  ...
  ROS_DEBUG_STREAM("AMCL: use_map_topic: " << use_map_topic_ << " first_map_only: " << first_map_only_);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(pf_init_)
  {
    ...
    if (update_dt < ros::Duration(update_min_dt_)) {
      update = false; // too fast
    }
  ...
  if(resampled || force_publication)
  {
    ...
    if(max_weight > 0.0)
    {
      ...
      ROS_DEBUG_THROTTLE(1.0, "%d: Rates: input: %2.1f output: %2.1f",
               update_count_,
               1.0/laser_scan_dt_avg_,
               1.0/update_dt_avg_);

      ROS_DEBUG_THROTTLE(1.0, "New pose: %6.3f %6.3f %6.3f",
               hyps[max_weight_hyp].pf_pose_mean.v[0],
               hyps[max_weight_hyp].pf_pose_mean.v[1],
               hyps[max_weight_hyp].pf_pose_mean.v[2]);
...
AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped& msg)
{
  ...
  ROS_INFO_STREAM("laser_model_type_: " << laser_model_type_);
