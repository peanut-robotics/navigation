...
bool AMCLOdom::UpdateAction(pf_t *pf, AMCLSensorData *data)
{
  ...
  case ODOM_MODEL_OMNI:
  {
    ...
    double trans_hat_stddev = (alpha3 * (delta_trans_noise*delta_trans_noise) +
                               alpha1 * (delta_rot_noise*delta_rot_noise));
    double rot_hat_stddev = (alpha4 * (delta_rot_noise*delta_rot_noise) +
                             alpha2 * (delta_trans_noise*delta_trans_noise));
    double strafe_hat_stddev = (alpha1 * (delta_rot_noise*delta_rot_noise) +
                                alpha5 * (delta_trans_noise*delta_trans_noise));
  ...
  case ODOM_MODEL_DIFF:
  {
    ...
    if (delta_rot1_noise < min_rot) {
      delta_rot1_noise = min_rot;
    }
    if (delta_rot2_noise < min_rot) {
      delta_rot2_noise = min_rot;
    }
    ...
    for (int i = 0; i < set->sample_count; i++)
    {
      ...
      delta_rot1_hat = angle_diff(delta_rot1,
                                  pf_ran_gaussian(this->alpha1*delta_rot1_noise*delta_rot1_noise +
                                                  this->alpha2*delta_trans_noise*delta_trans_noise));
      delta_trans_hat = delta_trans -
              pf_ran_gaussian(this->alpha3*delta_trans_noise*delta_trans_noise +
  ...
  case ODOM_MODEL_OMNI_CORRECTED:
  {
    ...
    // Precompute a couple of things
    double trans_hat_stddev = sqrt( alpha3 * (delta_trans_noise*delta_trans_noise) +
                                    alpha4 * (delta_rot_noise*delta_rot_noise) );
    double rot_hat_stddev = sqrt( alpha1 * (delta_rot_noise*delta_rot_noise) +
                                  alpha2 * (delta_trans_noise*delta_trans_noise) );
    double strafe_hat_stddev = sqrt( alpha4 * (delta_rot_noise*delta_rot_noise) +
                                     alpha5 * (delta_trans_noise*delta_trans_noise) );
    ...
    for (int i = 0; i < set->sample_count; i++)
    {
      ...
      delta_rot1_hat = angle_diff(delta_rot1,
                                  pf_ran_gaussian(sqrt(this->alpha1*delta_rot1_noise*delta_rot1_noise +
                                                       this->alpha2*delta_trans_noise*delta_trans_noise)));
      delta_trans_hat = delta_trans -
              pf_ran_gaussian(sqrt(this->alpha3*delta_trans_noise*delta_trans_noise +
                                   this->alpha4*delta_rot1_noise*delta_rot1_noise +
                                   this->alpha4*delta_rot2_noise*delta_rot2_noise));
      delta_rot2_hat = angle_diff(delta_rot2,
